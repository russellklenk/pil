...
{
    RUNTIME_MODULE d3dcompiler_dll;
    int                     result;

    assert(dispatch != NULL);
    RuntimeModuleInit(&d3dcompiler_dll);

    result = RuntimeModuleLoad(&d3dcompiler_dll, "d3dcompiler_47.dll");
...
{
    return RuntimeModuleIsValid(&dispatch->ModuleHandle_D3DCompiler);
}
...
)
{
    ...
    dispatch->D3DWriteBlobToFile              = D3DWriteBlobToFile_Stub;
    RuntimeModuleUnload(&dispatch->ModuleHandle_D3DCompiler);
}
